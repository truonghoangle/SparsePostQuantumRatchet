...
This definition enables stating mathematical specifications for functions like `Poly.zero`,
`Poly.add_assign`, `Poly.compute_at`, and `Poly.lagrange_interpolate` in terms of the algebraic
properties of `GF216[X]`.
-/
...
This is the semantic content of `Poly.zero`: any `Poly` whose `coefficients` list has length 0 maps
to `0 : GF216Poly`.

Note: `Poly.zero_spec` (in `Spqr/Specs/Encoding/Polynomial/Poly/Zero.lean`) now directly states the
mathematical postcondition:

  `zero capacity ⦃ fun result => result.toGF216Poly = 0 ⦄`

i.e. the Rust function `Poly::zero(capacity)` produces a value whose mathematical interpretation is
the zero polynomial in GF(2¹⁶)[X].  This theorem remains useful as a standalone lemma for any
`Poly` with an empty coefficient vector, regardless of how it was constructed.
-/
...
This parallels `natToBinaryPoly_coeff` from `Spqr.Math.Gf`, lifting the coefficient characterization
from the bit level (GF(2) polynomials representing individual field elements) to the polynomial
level (GF(2¹⁶) polynomials whose coefficients are field elements).
-/
...
This is a direct corollary of `listToGF216Poly_coeff` and captures the key invariant: the
`Vec<GF16>` length bounds the support of the polynomial.  In particular, the Rust constraint
`coefficients.len() ≤ MAX_INTERMEDIATE_POLYNOMIAL_DEGREE_V1 + 1` implies that the polynomial has
degree at most `MAX_INTERMEDIATE_POLYNOMIAL_DEGREE_V1`.
-/
...
The zero element has `value.val = 0`, which encodes the natural number `0`.  By
`natToBinaryPoly_zero` from `Spqr.Math.Gf`, `natToBinaryPoly 0 = 0`, and the ring homomorphism
...
The element with `value.val = 1` encodes the natural number `1`, which corresponds to the constant
polynomial `1` in GF(2)[X], and maps to `1 : GF216` via `BinaryPoly.toGF216`.
-/
...
Since `GF216 = GaloisField 2 16` is a field extension of `ZMod 2`, it inherits characteristic 2.
This is the fundamental fact that makes polynomial addition in the Rust implementation
(coefficient-wise XOR) correspond to polynomial addition in `GF216[X]`.
-/
...
This connects `Poly.compute_at` (which evaluates using Horner's method in the implementation) to the
mathematical `Polynomial.eval` function from Mathlib.  The correspondence is:

  `Poly.evalAt p x = Polynomial.eval (x.toGF216) (p.toGF216Poly)`

which says "evaluating the Poly at GF16 point x in the implementation equals evaluating the
corresponding GF216[X] polynomial at the corresponding GF216 point".
-/
...
For any `Poly` with an empty coefficient list (such as the result of `Poly.zero`), evaluation at any
GF16 point yields `0`.  This is the evaluation-level counterpart of `Poly.toGF216Poly_eq_zero`.
-/
...
Evaluating `listToGF216Poly cs` at point `a` yields the sum `∑ᵢ GF16.toGF216(csᵢ) · a ^ i`,
connecting the Mathlib `eval` function to the explicit coefficient-power-sum computation used in the
implementation's `Poly.compute_at`.

This is the key linking lemma for verifying `Poly.compute_at`: it says the mathematical evaluation
of the polynomial agrees with the sum-of-powers formula that the Rust function computes.
-/
...
If `cs ++ [a]` is the extended list, then `listToGF216Poly (cs ++ [a]) =
  listToGF216Poly cs + C(a.toGF216) · X^(cs.length)`.

This is the algebraic content of `Vec::push` on the coefficient vector: appending a new coefficient
extends the polynomial by one degree. It is the key invariant for verifying
`Poly.lagrange_interpolate_prepare` and `Poly.lagrange_interpolate_complete`, which build
polynomials by successively pushing coefficients.
-/
...
If two coefficient lists have the same length and their GF16 values are added pointwise (via XOR in
the implementation), the resulting polynomial is the sum of the two input polynomials.  This is the
algebraic specification for `Poly.add_assign` when both polynomials have the same degree.
-/
