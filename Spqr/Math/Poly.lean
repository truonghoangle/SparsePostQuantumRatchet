...
* `GF216Poly` for the type abbreviation `GF216[X]`, matching the
  pattern used by `BinaryPoly` in `Spqr.Math.Gf2Poly.Basic`.
* `listToGF216Poly` for the canonical map from `List GF16` to
  `GF216Poly`, matching the pattern of `natToBinaryPoly`.
* `Poly.toGF216Poly` for the canonical bridge from the
  Aeneas-extracted `Poly` to `GF216Poly`, matching the pattern of
  `Nat.toGF216`.
* `GF216Poly.neg_eq` and `GF216Poly.sub_eq_add` for the basic
  algebraic facts in characteristic 2, matching `BinaryPoly.neg_eq`
  and `BinaryPoly.sub_eq_add`.
* `GF216.two_eq_zero` for the characteristic-2 fact in `GF216`.

Note: this development is intended to be upstream-friendly so that
it can be reused by other projects working with the same Galois
field.
...
`listToGF216Poly [] = 0` because `Fin 0` is empty, so the sum
...
This is a direct corollary of `listToGF216Poly_coeff` and captures
...
`-a = a`.  This parallels `BinaryPoly.neg_eq` from `Spqr.Math.Gf`,
now lifted from `BinaryPoly` to `GF216Poly`. -/
...
Direct consequence of `GF216Poly.neg_eq`: `a - b = a + (-b) = a + b`.
This mirrors `BinaryPoly.sub_eq_add` from `Spqr.Math.Gf`, and is the
