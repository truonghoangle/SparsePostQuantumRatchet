...
/-!
# Spec theorem for `spqr::proto::pq_ratchet::v1_state::unchunked::{impl core::clone::Clone for Ct1Sent}::clone`

`Ct1Sent` is the protobuf-generated product (record) type

  `Ct1Sent ≃ { epoch : u64 ; auth : Option<Authenticator> ; hdr : Vec<u8> ; es : Vec<u8> ; ct1 : Vec<u8> }`

representing the serialised form of the "CT1 sent" state in the unchunked ratchet protocol.  After
the first ciphertext (CT1) has been sent, this state records the epoch at which the send occurred,
an optional authenticator for MAC verification, the ML-KEM header, the encapsulation state, and the
first ciphertext.  The five fields are:
  ...
  • `hdr : Vec<u8>` — the ML-KEM header bytes (`bytes` in proto3).
...
The Rust source struct (src/v1/unchunked/send_ct.rs, lines 58–69):
  ```rust
...
The protobuf schema (src/proto/pq_ratchet.proto, lines 107–113, inside `V1State.Unchunked`):
  ```proto
  message Ct1Sent {
    uint64 epoch = 1;
...
The Lean type definition (SrcTranslated/Types.lean, lines 331–339):
  ```lean
...
The compiler-derived `Clone::clone` implementation proceeds field-by-field:
  ...
  4. `CloneVec.clone CloneU8` for `es` (identity clone on `Vec<u8>`).
...
`Copy`, the element-wise `Vec<u8>` clone is the identity for all three byte-vector fields (`hdr`,
`es`, `ct1`).  Therefore the whole struct-level clone is observationally the identity:
  `clone(self) = ok self`
...
namespace spqr.proto.pq_ratchet.v1_state.unchunked.Ct1Sent
...
/--
**Spec theorem for `proto.pq_ratchet.v1_state.unchunked.Ct1Sent.Insts.CoreCloneClone.clone`**:

• Takes a `Ct1Sent` value `self` wrapping five fields:
...
• Delegates to field-wise cloning:
    ...
    - `alloc.vec.CloneVec.clone CloneU8` for `ct1`
      (identity clone on `Vec U8` — see `clone_vec_u8_spec`).
• Returns the resulting `Ct1Sent` unchanged.

The Aeneas-extracted Lean definition (SrcTranslated/Funs.lean, lines 586–600):
  ```lean
...
• The function always succeeds (no panic / no error) for any `Ct1Sent` input, since
...
• The result is propositionally equal to the input:
    `clone(self) = ok self`
  i.e. the derived `Clone` implementation is observationally the identity on `Ct1Sent`.
...
-/
@[step]
theorem clone_spec (self : proto.pq_ratchet.v1_state.unchunked.Ct1Sent) :
    Insts.CoreCloneClone.clone self ⦃ (result : proto.pq_ratchet.v1_state.unchunked.Ct1Sent) =>
      result = self ⦄ := by
...
end spqr.proto.pq_ratchet.v1_state.unchunked.Ct1Sent
