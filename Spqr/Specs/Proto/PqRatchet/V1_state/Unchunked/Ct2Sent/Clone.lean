...
/-!
# Spec theorem for `spqr::proto::pq_ratchet::v1_state::unchunked::{impl core::clone::Clone for Ct2Sent}::clone`

`Ct2Sent` is the protobuf-generated product (record) type

  `Ct2Sent ≃ { epoch : u64 ; auth : Option<Authenticator> }`

representing the serialised form of the "CT2 sent" state in the unchunked ratchet protocol.  After
the second ciphertext (CT2) has been sent, this state records the epoch at which the send occurred
and an optional authenticator for MAC verification.  The two fields are:
  • `epoch : u64` — the epoch counter at which CT2 was sent (`uint64` in proto3).
...
The Rust source struct (src/v1/unchunked/send_ct.rs, lines 84–88):
  ```rust
...
The protobuf schema (src/proto/pq_ratchet.proto, lines 121–124, inside `V1State.Unchunked`):
  ```proto
  message Ct2Sent {
    uint64 epoch = 1;
...
The Lean type definition (SrcTranslated/Types.lean, lines 320–322):
  ```lean
...
namespace spqr.proto.pq_ratchet.v1_state.unchunked.Ct2Sent
...
/--
**Spec theorem for `proto.pq_ratchet.v1_state.unchunked.Ct2Sent.Insts.CoreCloneClone.clone`**:

• Takes a `Ct2Sent` value `self` wrapping two fields:
    - `epoch : U64` — the epoch counter at which CT2 was sent.
...
• Returns the resulting `Ct2Sent` unchanged.

The Aeneas-extracted Lean definition (SrcTranslated/Funs.lean, lines 548–559):
  ```lean
...
• The function always succeeds (no panic / no error) for any `Ct2Sent` input, since
  `CloneU64.clone` is total and the `Option<Authenticator>` clone is total (see
  `Authenticator/Clone.lean`).
• The result is propositionally equal to the input:
    `clone(self) = ok self`
  i.e. the derived `Clone` implementation is observationally the identity on `Ct2Sent`.
...
-/
@[step]
theorem clone_spec (self : proto.pq_ratchet.v1_state.unchunked.Ct2Sent) :
    Insts.CoreCloneClone.clone self ⦃ (result : proto.pq_ratchet.v1_state.unchunked.Ct2Sent) =>
      result = self ⦄ := by
...
end spqr.proto.pq_ratchet.v1_state.unchunked.Ct2Sent
