...
(`proto.pq_ratchet.v1_state.unchunked.HeaderSent`) used for network serialization or
persistent storage.
...
1. Delegates to `Authenticator.into_pb` to convert the embedded `auth` field from
   ...
   in `some`.
2. Copies the remaining fields (`epoch`, `ek`, `dk`) directly into the protobuf struct.
...
**Source**: spqr/src/v1/unchunked/send_ek/serialize.rs (lines 26:4-33:5)
-/
...
• Takes a `self : v1.unchunked.send_ek.HeaderSent` containing an epoch counter (`U64`),
  an authenticator (`authenticator.Authenticator`), an encapsulation key (`ek : Vec U8`),
  and a decapsulation key (`dk : Vec U8`).
...
• Returns a `proto.pq_ratchet.v1_state.unchunked.HeaderSent` with:
