...
`EkSent.into_pb` converts an `EkSent` from the in-memory Rust form
(`v1.unchunked.send_ek.EkSent`) into the protobuf form
(`proto.pq_ratchet.v1_state.unchunked.EkSent`) used for network serialization or
persistent storage.
...
1. Delegates to `Authenticator.into_pb` to convert the embedded `auth` field from
   ...
   in `some`.
2. Copies the remaining fields (`epoch`, `dk`) directly into the protobuf struct.
...
**Source**: spqr/src/v1/unchunked/send_ek/serialize.rs (lines 50:4-56:5)
-/
...
• Takes a `self : v1.unchunked.send_ek.EkSent` containing an epoch counter (`U64`),
  an authenticator (`authenticator.Authenticator`), and a decapsulation key (`dk : Vec U8`).
...
• Returns a `proto.pq_ratchet.v1_state.unchunked.EkSent` with:
