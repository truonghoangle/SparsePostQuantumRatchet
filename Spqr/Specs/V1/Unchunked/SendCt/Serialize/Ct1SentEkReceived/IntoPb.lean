...
`Ct1SentEkReceived.into_pb` converts a `Ct1SentEkReceived` from the in-memory Rust form
(`v1.unchunked.send_ct.Ct1SentEkReceived`) into the protobuf form
(`proto.pq_ratchet.v1_state.unchunked.Ct1SentEkReceived`) used for network serialization or
persistent storage.
...
3. Copies the remaining fields (`epoch`, `es`, `ek`) directly into the protobuf struct.
...
**Source**: spqr/src/v1/unchunked/send_ct/serialize.rs (lines 74:4-82:5)
-/
...
• Takes a `self : v1.unchunked.send_ct.Ct1SentEkReceived` containing an epoch counter (`U64`),
  an authenticator (`authenticator.Authenticator`), an encoded secret (`es : Vec U8`),
  an encapsulation key (`ek : Vec U8`), and a first ciphertext (`ct1 : Vec U8`).
...
• Returns a `proto.pq_ratchet.v1_state.unchunked.Ct1SentEkReceived` with:
  ...
  - `ek` copied directly from `self.ek`,
