...
In the SPQR authenticator, `root_key` and `mac_key` are **jointly derived**
from the same HKDF-SHA-256 invocation. Specifically, in
`Authenticator::update` (src/authenticator.rs, lines 44–54):
...
A single 64-byte HKDF output is split at byte 32:
- `root_key` = first 32 bytes (`kdf_out[0..32]`)
- `mac_key`  = last 32 bytes  (`kdf_out[32..64]`)
