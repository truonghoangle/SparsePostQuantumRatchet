...
- `protobuf_decode_encode_roundtrip`: Protobuf decode-encode round-trip identity (canonical form)
...
/--
**Axiom: Protobuf decode-encode round-trip identity (canonical form)**.

Decoding a byte vector into a `PqRatchetState` and re-encoding the result yields the
original bytes.  This is the converse of `protobuf_encode_decode_roundtrip` and captures
the canonical-form property of prost's deterministic serialization: every valid protobuf
encoding is already in canonical form, so `encode(decode(s)) = s`.

**Justification**: The prost library uses deterministic serialization (fixed field
ordering, no unknown-field preservation in generated code).  For messages generated by
prost, re-encoding a decoded message always reproduces the original byte sequence.

**References**:
- Protocol Buffers Language Guide v3 — deterministic serialization guarantees
- prost library documentation — canonical encoding
-/
axiom protobuf_decode_encode_roundtrip :
  ∀ (pb : proto.pq_ratchet.PqRatchetState) (s : alloc.vec.Vec U8),
    decode_state s = ok (core.result.Result.Ok pb) →
    proto.pq_ratchet.PqRatchetState.Insts.ProstMessageMessage.encode_to_vec
      pb = ok s
