...
The proof unfolds `add` to expose the underlying `add_assign` call and discharges the resulting goal
with `step*`, which applies the already-registered `add_assign_spec`.
