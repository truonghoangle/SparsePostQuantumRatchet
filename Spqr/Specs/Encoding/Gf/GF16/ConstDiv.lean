...
The proof applies `loop.spec_decr_nat` with measure `16 − i.val` and the inductive invariant that,
at any intermediate state `(square', out', i')` with `i.val ≤ i'.val ≤ 16`,
...
The active step is discharged via `const_div_loop_body_spec` (registered `@[step]` above); the
routine commutative-ring exponent identities

  `(square^(2^k))² = square^(2^(k+1))`,
  `square^(2^(k+1) − 2) · square^(2^(k+1)) = square^(2^(k+2) − 2)`,

are settled by `omega` on the exponents (after `pow_succ` rewriting). The `done` branch forces
`i'.val = 16`, at which point the invariant already coincides with the postcondition (because `16 −
i.val + 1 = 17 − i.val`).
