...
`COMPLETE_POINTS_POLYS_34` (line 504) specialises `lagrange_polys_for_complete_points` to `N = 34`,
precomputing the Lagrange basis polynomials for evaluation points `0, 1, …, 33` in GF(2¹⁶) with
`y = GF16::ONE`.

The postcondition is inherited from `lagrange_polys_for_complete_points_spec`: there exists
`ones1` of size 34 with `ones1[j].x.toGF216 = Nat.toGF216 j`, `ones1[j].y = GF16::ONE`, and
each `result[j]` equals the standard Lagrange basis polynomial for these points.

In GF(2¹⁶) the points `0, 1, …, 33` are pairwise distinct, so the Fermat-inverse scaling yields
...
Evaluates successfully (specialisation of `lagrange_polys_for_complete_points` at `N = 34`).
Each `result[j]` is the `j`-th scaled Lagrange basis polynomial for the complete points
`0, 1, …, 33` with `y = GF16.ONE`. -/
