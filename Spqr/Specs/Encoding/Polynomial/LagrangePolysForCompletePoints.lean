...
Specifies one iteration of the initialisation loop (lines 477–482) in
`lagrange_polys_for_complete_points`.  The loop sets `ones[i].x.value = i as u16` and keeps
    ...
    (inv := fun (p : (Array (PolyConst N) N) × Usize) =>
        ...
        (∀ (j : Nat), i ≤ j → j < i' →
          ∀ (hj : j < out'.length) (hjo : j < ones.length),
            listToGF216Poly (out'.val[j]).coefficients.val =
              C ((ones.val[j]!).y.toGF216 *
                  (lagrangeDenomProd (ones[j]!).x
                    (ones.val.take N.val) 0) ^ (2 ^ 16 - 2)) *
                condProdLinearFactors (ones[j]!).x
