...
The extracted Lean function `encoding.polynomial.PolyEncoder.into_pb_loop0_loop0` is the inner
coefficient-serialization loop inside `PolyEncoder::into_pb`.  Given a vector `pts` of GF(2¹⁶)
elements (the `.value` entries of a `Point`), a `Range<usize>` iterator over `0..pts.len()`, and
the current output byte vector `v`, the loop repeatedly invokes
`encoding.polynomial.PolyEncoder.into_pb_loop0_loop0.body`, which at each step:

  1. Calls `next` on the `Range<usize>` iterator to obtain the current index `i`.
  2. Retrieves the `i`-th GF(2¹⁶) element `pt = pts[i]`.
  3. Converts `pt.value : u16` to its 2-byte big-endian representation via `u16::to_be_bytes`.
  4. Appends those bytes to `v` via `Vec::extend_from_slice`.

**Loop invariant**: after processing iterations up to `iter'`, the output byte vector satisfies:

  * `out'.val.length = 2 * iter'.start.val` — each GF(2¹⁶) element contributes exactly 2 bytes.
  ...
  * For every `j ∈ [0, iter'.start.val)`, the big-endian encoding holds:
      `∃ hi lo, out'.val[2*j]? = some hi ∧ out'.val[2*j+1]? = some lo ∧
        hi.val * 256 + lo.val = (pts.val[j]!).value.val`

At loop termination (`iter'.start.val ≥ iter'.«end».val`), the output vector contains the
complete serialization of all GF(2¹⁶) coefficients in `pts.val[0..iter.«end».val]`.
...
serialized bytes are preserved when new bytes are appended.
-/
...
/-- **Spec theorem for `encoding.polynomial.PolyEncoder.into_pb_loop0_loop0`**:

The full inner serialization loop inside `PolyEncoder::into_pb`.  Given the GF(2¹⁶) element
vector `pts` (the `.value` entries of a `Point`), a range iterator `iter` over indices into
`pts`, and the current output byte vector `v`, the loop drives the body to completion and
returns the serialized byte vector.

• The function always succeeds (no panic) provided the preconditions hold: the iterator range
  end does not exceed the GF(2¹⁶) element vector length, the output vector length equals
  `2 * iter.start.val` (each previously processed element contributed exactly 2 bytes),
...
theorem loop_spec
    ...
    (h_out_len : v.val.length = 2 * iter.start.val)
    (h_start_le : iter.start.val ≤ iter.«end».val)
    (h_overflow : 2 * pts.val.length + 2 ≤ Usize.max)
    ...
    into_pb_loop0_loop0 iter pts v ⦃ (result : alloc.vec.Vec Std.U8) =>
      ...
      ∀ (j : Nat), j < iter.«end».val →
        ∃ (hi lo : Std.U8),
          ...
          hi.val * 256 + lo.val = (pts.val[j]!).value.val ⦄ := by
  ...
  · -- Step: the body preserves the invariant or produces the final result
    ...
    have h_body := body_spec pts iter' out' (by omega) (by omega)
