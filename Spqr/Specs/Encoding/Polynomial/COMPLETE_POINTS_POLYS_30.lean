...
The Rust constant `COMPLETE_POINTS_POLYS_30` (in `src/encoding/polynomial.rs`, line 503) is defined
as
...
It precomputes the 30-element array of Lagrange basis polynomials for the "complete points" —
evaluation points `0, 1, …, 29` in GF(2¹⁶) with `y`-coordinate `GF16::ONE`.  The extracted Lean
definition `encoding.polynomial.COMPLETE_POINTS_POLYS_30` is simply a specialisation of the generic
`lagrange_polys_for_complete_points` to `N = 30`:
...
there exists an intermediate point array `ones1` of size 30 such that:

  - **Complete points**: for each `j < 30`, `ones1[j].x.value.val = j` and
    `ones1[j].y = GF16::ONE`.  Equivalently, `ones1[j].x.toGF216 = Nat.toGF216 j`.

  - **Lagrange basis polynomials**: for each `j < 30`,
      `listToGF216Poly (result[j].coefficients.val) =
         C (ones1[j].y.toGF216 *
             (lagrangeDenomProd ones1[j].x (ones1.take 30) 0) ^ (2¹⁶ − 2)) *
           condProdLinearFactors ones1[j].x (ones1.take 30) 0`
    which is the `j`-th term of the standard Lagrange interpolation formula for the
    points `ones1[0], …, ones1[29]`.

When the `x`-coordinates are pairwise distinct (which they are for the "complete points"
`0, 1, …, 29` in GF(2¹⁶)), the denominator `lagrangeDenomProd` is nonzero and the Fermat-style
...
**Source**: spqr/src/encoding/polynomial.rs (line 503)
-/
...
• The constant always evaluates successfully (no panic), since it is a specialisation of
  `lagrange_polys_for_complete_points` at `N = 30`, which succeeds for any `N` satisfying
  `0 < N` and `N ≤ 65536`.

• There exists an intermediate point array `ones1` of size 30 such that:

  - **Complete points** (every position `j < 30` has been initialised):
  ...
  - **Lagrange basis polynomials** (every position `j < 30` has been filled):
      `listToGF216Poly (result[j].coefficients.val) =
         C (ones1[j].y.toGF216 *
             (lagrangeDenomProd ones1[j].x (ones1.val.take 30) 0) ^ (2¹⁶ − 2)) *
           condProdLinearFactors ones1[j].x (ones1.val.take 30) 0`
    Each `result[j]` is the `j`-th term of the standard Lagrange interpolation formula
    for the points `ones1[0], …, ones1[29]`.

**Source**: spqr/src/encoding/polynomial.rs (line 503)
-/
@[step]
theorem COMPLETE_POINTS_POLYS_30_spec :
    COMPLETE_POINTS_POLYS_30
      ⦃ result =>
        ∃ (ones1 : Array Pt 30#usize),
          (∀ (j : Nat), j < (30#usize).val →
              ...
              listToGF216Poly (result.val.get ⟨j, hj⟩).coefficients.val =
                C ((ones1.val.get ⟨j, hjo⟩).y.toGF216 *
                    (lagrangeDenomProd (ones1.val.get ⟨j, hjo⟩).x
                      (ones1.val.take (30#usize).val) 0) ^ (2 ^ 16 - 2)) *
                  condProdLinearFactors (ones1.val.get ⟨j, hjo⟩).x
                    (ones1.val.take (30#usize).val) 0) ⦄ := by
