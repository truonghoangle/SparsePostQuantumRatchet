...
`encoding.polynomial.MAX_INTERMEDIATE_POLYNOMIAL_DEGREE_V1`, the public
constant defining the highest degree polynomial that will appear in
intermediate calculations for Protocol V1 of the Sparse Post-Quantum
Ratchet.

In Protocol V1, Lagrange interpolation is performed over at most 36
evaluation points, producing a polynomial of degree at most 35.
During the interpolation algorithm an intermediate product polynomial
of one degree higher is constructed, reaching degree 36.
This constant records that bound:
  `MAX_INTERMEDIATE_POLYNOMIAL_DEGREE_V1 = 36#usize`

The constant is used in `hax_lib` annotations to constrain array
sizes and loop bounds throughout the polynomial encoding layer.
It is unconditional and pure — its evaluation never fails and never
...
/-- **The underlying natural-number value of
`MAX_INTERMEDIATE_POLYNOMIAL_DEGREE_V1` is `36`**. -/
@[simp]
...
`encoding.polynomial.MAX_INTERMEDIATE_POLYNOMIAL_DEGREE_V1`**:

`MAX_INTERMEDIATE_POLYNOMIAL_DEGREE_V1` is the constant `36 : usize`,
recording the highest polynomial degree arising in intermediate
calculations during Protocol V1 Lagrange interpolation.
A polynomial of degree 36 has 37 coefficients, one more than the
36 coefficients of the maximum stored polynomial (degree 35).

Concretely:

  `MAX_INTERMEDIATE_POLYNOMIAL_DEGREE_V1.val = 36`

The proof unfolds the definition and reduces the `usize` literal
`36#usize` to its natural-number value.
