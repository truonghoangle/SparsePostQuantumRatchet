...
`encoding.polynomial.MAX_STORED_POLYNOMIAL_DEGREE_V1`, the public
constant defining the highest degree polynomial that will be stored
for Protocol V1 of the Sparse Post-Quantum Ratchet.
...
`MAX_STORED_POLYNOMIAL_DEGREE_V1` is `35`**. -/
@[simp]
...
`encoding.polynomial.MAX_STORED_POLYNOMIAL_DEGREE_V1`**:

`MAX_STORED_POLYNOMIAL_DEGREE_V1` is the constant `35 : usize`,
recording the highest polynomial degree stored in Protocol V1.
A polynomial of degree 35 has 36 coefficients, matching the
maximum number of Lagrange interpolation points used in V1.
...
`35#usize` to its natural-number value.
